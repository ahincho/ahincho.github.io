...
- 42036ce: Put Angular, Azure, Terraform and GitHub Actions on the sharing card, and give
  the card a URL that changes when the card does. The technology line was one row
  of SVG text, which does not wrap — four more names would have run off the edge
  of the 1200px canvas — so it is now two rows, stack above and platform below.
  The card also sat at `/og.png` forever, so scrapers that had cached it never
  asked again: Discord was still serving the version from before "AI Engineer" was
  in the role while reading the new title off the same page. The script that draws
  the card now writes its fingerprint beside it, and the URL carries it.

## 0.8.1

### Patch Changes

- 4017f03: Print how many questions the assistant has answered in its own case study,
...
- 8ef2834: Count how often each case study is read. Every project page now records a read
...
- 1ca69b0: The footer read "Astro· GitHub· LinkedIn". The separator sat at the start of its
...
- 623f78e: The hero opened with a flat 4rem above it on every screen under 900px. Stacked
  on the 65px sticky header, that left a phone with 129 pixels — a sixth of the
  screen — before the first word, and pushed the terminal off the fold entirely.
  The space is fluid now: 36px on a phone, 54px on a tablet, unchanged on a
  desktop, where there was room for it all along.
  
  The GitHub and LinkedIn icons in the hero were 20×20 targets, under the 24 WCAG
  asks for and well under what a thumb needs. The icons are the same size; the
  target around them is 39×39, grown with padding and given back with a negative
  margin so nothing on the line moved.
...
- 938d0ec: The hero terminal now reads `metrics.log` rather than `visits.log`, and prints
...
- cf94233: The chat Worker now keeps a daily roll-up of how many questions it was asked and
...
- 18bf44b: Let the portfolio keep its own numbers. Each project now carries a like anyone
  can give and take back, the hero terminal shows unique visits, and CV downloads
  are counted without being displayed. The counter Worker grew a reactions family
  alongside the counters: a reaction is one row per person, so the total is a
  count of those rows and undoing a like cannot leave the number and the rows
  disagreeing, and the endpoint reports which likes are yours so a fresh page load
  never shows a pressed button as unpressed.
...
- 23789f5: Name AI in the role — "Full Stack, DevSecOps & AI Engineer" — and give the claim
  something to stand on: the chat assistant is now a project of its own, with a
  case study in both languages covering how it stays anchored to the site's
  content, how it survives a prompt injection, and what every setting was measured
  at. It is the only project on the portfolio a reader can use rather than read
  about.
...
- b91680c: Ask the second model alongside the first when the first is slow, instead of
